...
- (void) adjustDays: (int)days
{
	...
	CFCalendarAddComponents ( [self calendar], &newTime, 0, (const char *)format, days );
...
- (void) adjustMonths: (int)months
{
	...
	if ( [THCalendarInfo defaultRoundingRule] == SDCalendarExactCountRule )
	{
		...
		CFCalendarAddComponents ( [self calendar], &newTime, 0, (const char *)dFormat, (months * 30) );
			
	} else {
		
		const unsigned char mFormat[] = "M";
		CFCalendarAddComponents ( [self calendar], &newTime, 0, (const char *)mFormat, months );		
	}
...
- (void) adjustYears: (int)years
{
	...
	CFCalendarAddComponents ( [self calendar], &newTime, 0, (const char *)format, years );
	[self setAbsoluteTime: newTime];
}
...
- (int)weeksInMonth
{
	...
	//		//(@"In this region, August 1st, 2008 is the ordinal %d of the month", weekNumber);
	//
